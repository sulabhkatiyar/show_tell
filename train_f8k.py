...
emb_folder ='path_to_embedding_folder_if_using_ensemble'  # if using a whole ensemble
use_embeddings_ensemble = False

#Also choose the value of 'embeddings_ensemble_available' in models.py file 
embedding_folder = 'path_to_embedding_folder_if_using_single_embedding'
use_pretrained_embeddings = True  #want to use pre-trained embeddings?
emb_file_name= 'f8kwiki.simplefasttext.txt'  # 'f8k_word2vec300.txt', 'f8kglove.840B.300d.txt', 'f8kwiki.simplefasttext.txt'
embeddings_file = embedding_folder + emb_file_name

fine_tune_embeddings = False      #want to fine tune the embeddings too?
slow_down_embeddings = False
...
#IMPORTANT TO DO: Find best Learning rate for embedding layer post phase 1 training. 
...
def main():
    ...
    if use_pretrained_embeddings == True:
        embeddings, emb_dim=load_embeddings(embeddings_file, word_map)
    elif use_embeddings_ensemble == True:
        embeddings, emb_dim=get_ensemble_embeddings(emb_folder, word_map)
    ...
    if checkpoint is None:
        ...
        if use_pretrained_embeddings==True or use_embeddings_ensemble == True:   #using pre-trained embeddings
            decoder.load_pretrained_embeddings(embeddings)
            decoder.fine_tune_embeddings(fine_tune_embeddings)
            
        # Following if-else block if for training the embedding layer with slower learning rate.
        # To be used with a pre-trained embedding only.
        if slow_down_embeddings == False:
            decoder_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, decoder.parameters()),
                                             lr=decoder_lr)
        else:
            print('Slowing embeddings down...')
            for param in decoder.embedding.parameters():
                param.requires_grad=True
            decoder_optimizer = torch.optim.Adam([{'params': decoder.embedding.parameters(), 'lr':1e-3},
                                                 {'params': decoder.dropout.parameters()},
                                                 {'params': decoder.decode_step.parameters()},
                                                 {'params': decoder.init_h.parameters()},
                                                 {'params': decoder.init_c.parameters()},
                                                 {'params': decoder.fc.parameters()}], lr=decoder_lr)
        ...
        # The following 'if' block is designed to be used when using slow learning rate for embeddings.
        # Designed to be used with a checkpoint in a two-stage learning where the fist stage only trains the decoder 
        # and after validation loss stops decreasing, the encoder and word embeddings (or any one ) is trained.
        
        if slow_down_embeddings == True:
            for param in decoder.embedding.parameters():
                param.requires_grad=True
            last_lr = checkpoint['decoder_optimizer'].param_groups[0]['lr']
            decoder_optimizer = torch.optim.Adam([{'params': decoder.embedding.parameters(), 'lr':1e-4},
                                                 {'params': decoder.final_embeddings.parameters()},
                                                 {'params': decoder.dropout.parameters()},
                                                 {'params': decoder.decode_step.parameters()},
                                                 {'params': decoder.init_h.parameters()},
                                                 {'params': decoder.init_c.parameters()},
                                                 {'params': decoder.features_tensor.parameters()},
                                                 {'params': decoder.fc.parameters()}], lr=last_lr)

        # This is for the old way in which embeddings were trained with decoder_lr along with rest of the decoder
        else:
            if use_pretrained_embeddings==True:  
                decoder.fine_tune_embeddings(fine_tune_embeddings) 
    ...
    randomperspective = transforms.RandomPerspective(distortion_scale=0.1)
