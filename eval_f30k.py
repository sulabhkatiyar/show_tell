...
word_map_file = data_folder + '/WORDMAP_flickr30k_5_cap_per_img_5_min_word_freq.json'  # word map, ensure it's the same the data was encoded with and the model was trained with
...
def evaluate(beam_size):
    ...
    if captions_dump==True:
        with open('generated_captions_f30k.json', 'w') as gencap:
            json.dump(captions_dict, gencap)
        save_captions_mscoco_format(word_map_file,references,hypotheses,image_names,str(beam_size)+'_f30ktest')
