...
class CaptionDataset(Dataset):
    ...
    def __getitem__(self, i):
        # Nth caption corresponds to the (N // captions_per_image)th image
        img = torch.FloatTensor(self.imgs[i // self.cpi] / 255.)
        if self.transform is not None:
            img = self.transform(img)

        # Get the image path and image name from it
        ...
        if self.split is 'TRAIN':
            ...
            # For validation of testing, also return all 'captions_per_image' captions 
